...
#Eclidean Distance Formula (allows for diagonal movement)
def h(p1, p2):
    x1, y1 = p1
    x2, y2 = p2
...
def draw_grid(win, rows, width):
    gap = width // rows # makes all the spaces
    for i in range(rows): # makes horizontal line
...
def main (win, width):
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if started:
                continue
            # make start,e nd, node or barrier
